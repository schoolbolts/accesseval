  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-6 flex-wrap items-center">
        ...
        <span className="text-sm font-body text-slate-600 ml-auto">{total} organizations</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="card p-8 text-center">
          <p className="text-sm font-body text-slate-600">No organizations found.</p>
        </div>
      ) : (
        <>
            ...
            <table className="w-full text-sm font-body">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50">
                  <th className="text-left py-2.5 px-4 text-xs font-semibold text-slate-600 uppercase tracking-widest">Organization</th>
                  <th className="text-left py-2.5 px-3 text-xs font-semibold text-slate-600 uppercase tracking-widest">Owner</th>
                  <th className="text-left py-2.5 px-3 text-xs font-semibold text-slate-600 uppercase tracking-widest">Plan</th>
                  <th className="text-left py-2.5 px-3 text-xs font-semibold text-slate-600 uppercase tracking-widest">Status</th>
                  <th className="text-center py-2.5 px-3 text-xs font-semibold text-slate-600 uppercase tracking-widest">Sites</th>
                  <th className="text-center py-2.5 px-3 text-xs font-semibold text-slate-600 uppercase tracking-widest">Users</th>
                  <th className="text-left py-2.5 px-3 text-xs font-semibold text-slate-600 uppercase tracking-widest">UTM</th>
                  <th className="text-left py-2.5 px-3 text-xs font-semibold text-slate-600 uppercase tracking-widest">Signed up</th>
                </tr>
              </thead>
              <tbody>
                {orgs.map((org) => (
                  <>
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3 text-slate-600 text-xs truncate max-w-[120px]">{org.utm || '—'}</td>
                      <td className="py-2.5 px-3 text-slate-600">{new Date(org.createdAt).toLocaleDateString()}</td>
                    </tr>
                    {expanded === org.id && (
                      <tr key={`${org.id}-detail`} className="border-b border-slate-100">
                        <td colSpan={8} className="px-4 py-4 bg-surface">
                          <div className="grid grid-cols-2 gap-4 text-xs font-body">
                            <div>
                              <p className="text-slate-600 mb-1 font-semibold uppercase tracking-widest">Sites</p>
                              {org.sites.length === 0 ? (
                                <p className="text-slate-600">No sites</p>
                              ) : (
                                <ul className="space-y-1">
                                  {org.sites.map((s, i) => (
                                    <li key={i} className="text-slate-600">
                                      {s.url.replace(/^https?:\/\//, '')}
                                      <span className="text-slate-600 ml-2">({s.cmsType}, {s.scanCount} scans)</span>
                                    </li>
                                  ))}
                                </ul>
                              )}
                            ...
                            <div>
                              <p className="text-slate-600 mb-1 font-semibold uppercase tracking-widest">Details</p>
