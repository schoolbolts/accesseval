...
export async function sendWeeklyDigests(): Promise<void> {
  ...
  for (const org of orgs) {
    ...
    const previousScan = await prisma.scan.findFirst({
      where: { siteId: org.site.id, status: { in: ['completed', 'partial'] }, id: { not: latestScan.id } },
      orderBy: { completedAt: 'desc' },
    });
    ...
    const issuesFixed = await prisma.siteIssue.count({
      where: { siteId: org.site.id, status: 'fixed', statusChangedAt: { gte: weekAgo } },
    });
